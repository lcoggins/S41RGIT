dglm <- function (data.set, dist="gamma", J=FALSE, write=FALSE,
          ...
          cols.bin=NA, cols.pos=NA,
          jack.noise=1, ig.scale=1)
{
    #updated 4/29/2010 by Paul Conn to allow different submodels for bernoulli and positive CPUE
    #cols.bin give the columns of predictor variables input dataset that are to be used for the bernoulli submodel 
    #  this MUST NOT include column 1, but MUST include column  2 (year effect)
    #cols.pos give the same for the positive submodel
    #not using these arguments will result in all predictors being used for that given submodel
    #Note that filtering still proceeds by removing records for both submodels for which ANY factor doesn't have a certain # of observations
    # that is, records may be removed from a submodel even when that factor is not used in the submodel's formula
    if(is.na(cols.bin[1])==1)cols.bin=c(2:ncol(data.set))
    if(is.na(cols.pos[1])==1)cols.pos=c(2:ncol(data.set))
    ...
    if(dist == 'lognormal')
    {
       formula1 <- as.formula(paste(paste("log(", names(data.set)[1], ")", "~",
                           sep = ""), paste(names(data.set)[cols.pos], sep = "",
                           collapse = "+")))
    }
    ...
    if(dist == 'gamma' || dist == 'inverse.gaussian')
    {
       formula1 <- as.formula(paste(paste(names(data.set)[1], "~", sep = ""),
                           paste(names(data.set)[cols.pos], sep = "", collapse = "+")))
    }

    formula2 <- as.formula(paste(paste(names(data.set)[1], "~", sep = ""),
                        paste(names(data.set)[cols.bin], sep = "", collapse = "+")))
...
get.effects <- function(glm.obj, target.col)
{
  ...
  col.num <- ifelse(glm.fam=="binomial", ncol(glm.data), ncol(glm.data)-1)    #weights are appended in pos submodel
  var.class <- NULL
  for (i in 1:col.num)
  {
...
# redefine data frame of positive records & weight vector (post-filter)
    posdat <- data.set[data.set[, 1] > 0, c(1,cols.pos)]
    posweights <- weight.vector[as.numeric(rownames(posdat))]
...
#----------------
    ...
    if (J==TRUE)
    {
        ...
        for (j in 1:jack)
        {
            ...
            jdat <- data.set[-j, ]
            jposdat <- jdat[jdat[, 1] > 0, c(1,cols.pos)]
...
# Create 'list' objects that will hold effects for both GLMs
  if (ncol(data.set)>2)
  {
      pos.eff.list <- as.list(rep(NA, length(cols.pos)-1))
      bin.eff.list <- as.list(rep(NA, length(cols.bin)-1))
      pos.eff.names <- as.list(rep(NA, length(cols.pos)-1))
      bin.eff.names <- as.list(rep(NA, length(cols.pos)-1))
      #index.eff.list <- as.list(rep(NA, length(cols.bin)-1))
      for (i in 1:(length(cols.pos)-1))
      {
      ...
      for (i in 1:(length(cols.bin)-1))
      {
          bin.eff.list[[i]] <- get.effects(bin.fit, (i+2))
          #bin.eff.list[[i]] <- get.effects(bin.fit, (i+2))
          bin.eff.names[[i]] <- names(bin.eff.list[[i]])
      }
  ...
  if (ncol(data.set)>2)
  {
    glmdelta.summary <- list(error.distribution = fam.out,
                             ...
                             pos.effects=pos.eff.list,
                             bin.effects=bin.eff.list,
  ...
  if (write==TRUE)
  {
      ...
      if (ncol(data.set)>2)
      {
         ...
         #}
         write(c("\n"), file="deltaGLM_output.txt", append=T)
         write(c("Positive submodel effects"), file="deltaGLM_output.txt", append=T)
         write(c("\n"), file="deltaGLM_output.txt", append=T)
         for (i in 1:length(pos.eff.list))
         {
            write.table(pos.eff.list[[i]], file="deltaGLM_output.txt",
                        #col.names=names(pos.eff.list[[i]]),
                        col.names=T,
                        quote=F, sep="\t", append=T)
         }
         write(c("\n"), file="deltaGLM_output.txt", append=T)
         write(c("Bernoulli submodel effects"), file="deltaGLM_output.txt", append=T)
         write(c("\n"), file="deltaGLM_output.txt", append=T)
         for (i in 1:length(bin.eff.list))
         {
            write.table(bin.eff.list[[i]], file="deltaGLM_output.txt",
                        #col.names=names(bin.eff.list[[i]]),
                        col.names=T,
                        quote=F, sep="\t", append=T)
         }
      ...
      write.table(as.data.frame(AIC.results), quote=F, sep="\t",
                  file="deltaGLM_output.txt", append=T)
      options("warn"=0)
      dput(posdat, file="datpos.filtered.rdat") #Added KWS
      dput(bindat, file="datbin.filtered.rdat") #Added KWS
  }
