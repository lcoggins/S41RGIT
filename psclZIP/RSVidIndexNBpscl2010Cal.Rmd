...
# Now Recalibrate 2010 Data 
  cal.prop=0.47
  table(boot.dat$y,boot.dat$fishseen)
  dat.2010=boot.dat[boot.dat$y==2010,]
  cal.fishseen=rep(NA,length(dat.2010$fishseen))
  for(i in 1:length(dat.2010$fishseen)){cal.fishseen[i]=rbinom(1,dat.2010$fishseen[i],cal.prop)}#;plot(cal.fishseen,dat.2010$fishseen,xlim=c(0,250),ylim=c(0,250))
  boot.dat[boot.dat$y==2010,1]= cal.fishseen 
...
axis(1,at=1:3,labels=2010:2012)
index = resvec/mean(resvec)
lines(index,type='l',ylim=c(0,4),col='green',lwd=2)
approxgam=c(.37,.11,.21)
lines(approxgam/mean(approxgam),lwd=2,col='blue')
legend('topright',legend=c('2010 Calibration Bootstrap ','Bootstrap CI','Initial ZINB Index Uncalibrated Data','Approximate Delta GAM Index'),lty=c(1,3,1,1),col=c('red','black','green','blue'),lwd=c(2,1,2,2))

save.image("//CCFHR-S-1534090/popdyn1/SEDAR/SEDAR 41/SEFISIndices/S41RGIT/ZINB2010Cal.RData")
```
